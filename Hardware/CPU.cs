...
public class CPU
{
    ...
    public void ExecuteNextInstruction()
    {
        ...
        {
            // Input - If the subtrahend is -1, read from the input 'register'
            case (-1, _, _):
    ...
    {
        // Subtract and store result in target memory address (minuend)
        Subtract(addressOfSubtrahend, addressOfMinuend);

        // If the result is less than or equal to zero, jump to the target address
        LessThanOrEqualJump(addressOfMinuend, addressOfJumpTarget);
    }

    private void Subtract(int addressOfSubtrahend, int addressOfMinuend)
    {
        memory.Write(addressOfMinuend, memory.Read(addressOfMinuend) - memory.Read(addressOfSubtrahend));
    }
    
    private void LessThanOrEqualJump(int addressOfMinuend, int addressOfJumpTarget)
    {
